# Child SoA telemetry-off selector, fresh attempt 2

## Scope

This is one wholly fresh, balanced, non-qualification telemetry-off selector of the count-gated Child SoA staging trial against the unchanged normal candidate.

Accepted semantic and count gate: `b1ab3bc` and `results/phase7/child_soa_trial_2026-09-09/count_attempt3/PARENT_VERDICT.md`.

The first selector attempt is terminal `ABORTED` at `eac8039`: its mode-444 normal artifact could not execute, no benchmark logic or timed row ran, and it contributes no input here. Nothing from its artifact or evidence paths may be modified, reused, or deleted.
...
Normal:
`/home/icly/Documents/tetris_ai_runner_results/phase7/child_soa_trial_2026-09-09/selector_attempt2_artifacts/tetris_profile_normal`
SHA-256 `1dccaa808044593bd97d6e8af09848ce8b19e75b9dcefa50d63a17a6381db765`

Child SoA:
`/home/icly/Documents/tetris_ai_runner_results/phase7/child_soa_trial_2026-09-09/selector_attempt2_artifacts/tetris_profile_child_soa`
...
`/home/icly/Documents/tetris_ai_runner/results/phase7/child_soa_trial_2026-09-09/count_attempt3/PARENT_VERDICT.md`
SHA-256 `94a95df71e382d59d216b7be8cc4ce5458a99a84f60aae36d154c9963878062d`
...
```text
bash /home/icly/Documents/tetris_ai_runner/results/phase7/child_soa_trial_2026-09-09/selector_attempt2/collect.sh
```

Collector SHA-256: `6527addc46a3ebd84b0c64196d040e9251363145cbf6509f2b0a5262d8850740`.

Verify and record the collector hash in `MANIFEST.txt`. No profile or prewarm command may be invoked outside the collector. Never invoke the collector twice, including after an error or provider failure.
...
The collector must begin from a clean tracked tree whose HEAD contains `eac8039`. It verifies all four frozen hashes, artifact modes, CPU 7 governor `performance`, CPU 7 energy preference `performance`, global boost `1`, CPU 15 initially online, and noninteractive CPU-control authority.
...
Run normal then SoA once each. Discard both stdout and stderr. Record full commands, timestamps, loads, CPU states, and exits.
...
No telemetry counter is used for selector calculation. Count identity was established separately at `b1ab3bc`.

## Selector calculation

For each pair calculate SoA divided by normal for `total_s` and `p95_ms`, regardless of execution order. Keep full decimal precision for threshold comparisons. Display ratios to at least five decimal places.
...
- `PRIMARY-SELECT-SOA`: both exact medians are at most `0.90` and both exact spreads are at most `0.04`.
...
`PRIMARY-SELECT-SOA` establishes a clear current-candidate win and permits only freezing a fresh controlled SoA-versus-legacy protocol. It does not itself authorize a legacy execution or any qualification or production change.

`FALLBACK-OWNER-REVIEW` stops for the owner's explicit fallback decision before any legacy evidence is consumed.

`NO-ADVANCE`, `BLOCKED`, or `ABORTED` stops the Child SoA line. No classification permits appending selector pairs or inferring performance from the earlier telemetry-on count rows.
