# Child SoA count gate, fresh attempt 2

## Scope

This is one fresh execution of the mandatory semantic and work-vector gate for the trial-only two-array Child staging layout. The prior provider failure produced no files or execution, and the archived 14:18Z attempt stopped `BLOCKED` before collection. Neither contributes a row to this attempt.
...
Blocked-attempt archive commit: `19ea6a1`.
...
- Execution HEAD contains `e02f476`, `369d447`, and `19ea6a1` and has no tracked or staged changes.
...
Store the raw files under `/home/icly/Documents/tetris_ai_runner_results/phase7/child_soa_trial_2026-09-09/trace_attempt2/`, make them read-only after collection, and record their hashes and sizes in `trace_summary.json`.
...
Write each unedited row to `runN_<normal|soa>.txt`. Write `rows.sha256`, `binaries.sha256`, `commands.txt`, `MACHINE.md`, `MANIFEST.txt`, `trace_summary.json`, and `summary.json`. Every recorded command and artifact path must use the frozen absolute spelling. `summary.json` must list every check with its observed values.
