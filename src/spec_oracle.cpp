// spec_oracle.cpp
// Phase 0 temporary tool. Independent hand-authored spec checks that are not
// generated by legacy code. Removed together with the legacy code after final
// acceptance.
//
// Hand-authored from the published Super Rotation System specification
// (tetris.wiki, "Super Rotation System"):
//   - the JLSTZ and I wall-kick tables in (x, y) test order, y-up
//   - first-valid candidate ordering, direct rotation first
//   - a scalar row-array board model with column-top collision, line clear,
//     and command replay semantics
//
// The legacy engine trims piece bounding boxes, so the mapping from an
// internal (x, y, r) status to occupied cells has a per-orientation anchor.
// The anchor is measured once from the engine at a reference status and is
// recorded here as the frame conversion. Everything else is compared against
// the hand-authored tables.
//
// Checks:
//   1. precomputed CW and CCW kick candidate sequences equal the published
//      tables in order, filtered by wall bounds
//   2. basic rotation equals the direct (identity-kick) candidate
//   3. direct 180 rotation reaches the opposite orientation pattern
//   4. row-array board transitions for hand-computed line clears
//   5. legacy-generated paths replayed through the scalar interpreter,
//      ending exactly on the searched landing placement
